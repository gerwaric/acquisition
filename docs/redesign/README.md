...
|---|---|---|---|
| rate-limit-core | Can a Rust client demonstrably honor the N-claims in `network-ground-truth.md` under burst load, as a single serialized gate? | cut: `spike/rate-limit-core` | [Yes offline for all four OAuth Known lanes; yes conditionally for the legacy Assumed lane](topics/rate-limit-core.md) |
