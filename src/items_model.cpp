...
QVariant ItemsModel::data(const QModelIndex& index, int role) const {
	...
	if (index.internalId() == 0) {
		...
		if (role == Qt::DisplayRole) {
			...
			auto const& bo = bo_manager_.GetTab(location.id());
			if (bo.IsActive())
		...
		if (location.IsValid() && location.type() == ItemLocationType::STASH) {
			if (role == Qt::BackgroundRole) {
				QColor backgroundColor(location.red(), location.green(), location.blue());
...
bool ItemsModel::setData(const QModelIndex& index, const QVariant& value, int role) {
	...
	if (role == Qt::CheckStateRole) {
		...
		// the layout
		std::string target_hash = location.id();
		auto row_count = rowCount();
		for (int i = 0; i < row_count; ++i) {
			auto match_index = this->index(i);
			if (search_.GetTabLocation(match_index).id() == target_hash)
