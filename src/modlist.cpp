...
void InitModList() {
	...
	mods_map.clear();
	for (auto& list : mods) {
		std::string first_mod = list[0];
		if (mod_strings.count(first_mod) > 0) {
			QLOG_WARN() << "InitModList(): duplicate mod:" << first_mod;
		} else {
			mod_strings.insert(first_mod);
			SumModGen gen = std::make_shared<SumModGenerator>(first_mod, list);
			mods_map.insert(std::make_pair(first_mod, gen.get()));
