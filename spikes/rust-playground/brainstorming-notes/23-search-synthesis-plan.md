...
| 5 | Under decision 15 the rulings are provisional. Input: note 31 whole, and its last section first — Astra's check holds seven corrections, K1–K7, that a row is harvested *with*, never without; K6 was written before the owner's answer (digest S59) and is settled by it. Ruling: candidate decision lines in registry form, a parking lot with triggers, the owner's verdicts verbatim; harvested into `decisions/search.md` (new area file, one index row in `CONTEXT.md`) | the owner, with Fable | note 28, then `decisions/search.md` | — |
