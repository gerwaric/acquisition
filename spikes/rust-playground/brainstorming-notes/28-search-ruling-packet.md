...
## 2b. Mechanism as recorded — moved to `search/DESIGN.md`

The contract detail recorded here at the harvest — by decision id, with
Astra's completions accepted as one voice — moved verbatim to
`search/DESIGN.md` on 2026-09-18, by the owner's choice of home (the
stage-5 audit, finding 6): it decides answers, so it is binding, and a
note is never an authority (P1). "§2b" in the sections below means that
document; the text as it stood here is this note at `a9ea0277`.
