...
Run under note 32 against `search/proposal-audit/audit-24.md`. Nothing
outside these rows was changed.

| id | answer | where |
| --- | --- | --- |
| F1 | fixed | §2: the string on the MCP marked as a departure from R14 (S195), on judgment |
| F2 | fixed | §3: 202 ms named as the parse alone; the store read stated as unmeasured; the 90 k trigger marked arithmetic |
| F3 | fixed | §1: the sum rule marked judgment, re-cited to S29, S49 and S27–S28; S48, S73 dropped |
| F4 | fixed | §2: S68 moved to the comparison rule it supports; the mean inside `sum` marked a judgment against its grain |
| F5 | fixed | §2: productions for `has:name`, `has:kind`, `is:flag`; kinds and flags defined |
| F6 | fixed | §2: the empty query; §1: several count names are one table each |
| F7 | gap | §3: the class vocabulary is the export's 103 and nothing above; OQ5 rewritten on a user's name; listed in §5 |
| F8 | fixed | §3: the totals' names; `total-res` in OQ1 and AQ2 |
| F9 | fixed | §1: the list closed at twenty-nine; §3: each field's source; `strength`/`dexterity` become totals; `colors` a multiset |
| F10 | fixed | §4.5: `weight` → `sum` cited to trade-query F2, `undigested`, as my reading |
| F11 | gap | §4.5: the 27 unsettled filters have no mapping; listed in §5 |
| F12 | fixed | §3: "Two relations carry the corpus"; the reference tables stand beside the contract as inputs |
| F13 | fixed | §1: `--expand` is C53's audit view |
| F14 | fixed | §3 and §9.4: the crate's home against the store park, handed to the owner |
| F15 | fixed | §3: both store needs stated as requests the read surface does not evidence; staleness conditional on the counter; the mid-answer ingest |
| F16 | fixed | §2: the bare-word default marked judgment; S113–S115 cited for the habit only |
| F17 | fixed | §5 OQ1: an `or` group of three comparisons, no `count` |
| F18 | fixed | §1: `show` and `--against` defined as the one-item view of `id:<x>`; §7 entry 10 |
| F19 | fixed | §7 entry 7: `count` stands on stance 2 and the trade mapping, used by none of the twelve |
| F20 | fixed | §7 entry 11: the floor is its justification |
| F21 | fixed | §7: entries 4–6, three lines; the inventory is eleven |
| F22 | fixed | §4.5: S67's undecided ids cross as both candidates in a `count` min 1 |
| F23 | fixed | §1: `--sort thing`, `--desc`; §5 AQ5 |
| F24 | fixed | §5 AQ5: one spelling; §1: `+#` and `#` are one template |
| F25 | fixed | §1: the sign convention stated as my choice, citing the contradictions entry; S4 joined by S71 for the markup half |
| F26 | gap | §4.1: twins on one item are summed; S44 dropped; listed in §5 |
| F27 | fixed | §2: name first, then text; quotes force text; redefining a name is an error |
| F28 | fixed | §5 AQ4: `id:` is contains like every field, a prefix being a substring |
| F29 | gap | §3: a keystroke after a store change pays the whole re-derive; listed in §5 |
| F30 | fixed | §3: S157's 162 ms named for what it built and read as an upper bound, a judgment |
| F31 | fixed | §1: the closure stated as the design's definition of "displays" |

The cold start is recounted with the audit: OQ1 is three calls, not two;
AQ5 stands at one because the tool's arguments are now in the
description.

What the audit taught me: the model held, but I had written its edges
— the names, the totals, the view arguments, the numbers from the
store — as things I knew, not things the page says, and a stranger
meets the page.

## Repairs — round 2

Run under note 32 against the second `search/proposal-audit/audit-24.md`
(at `cb2a6fdb`). F1, F4, F9 and F18 are left unanswered, as instructed.
Nothing outside these rows was changed.

| id | answer | where |
| --- | --- | --- |
| F2 | fixed | §2: one rule — on a closed value set `:` is a substring of the legal values, resolved values listed, text none contains an error (S199); §4.3's clause moved there |
| F3 | fixed | §6: call 1 returns lines only; call 3 writes its totals as `sum(…)` over the templates read; the count stays three |
| F5 | fixed | §1: a veiled placeholder is a line with no numbers (S12), so no comparison matches it |
| F6 | gap | §4.1: a shipped total lists its templates with zeros; a moved template does not fail loudly; §8's S111 qualified; listed in §5 |
| F7 | fixed | §2: S113–S115 dropped; the bare word stands as judgment |
| F8 | fixed | §3: socket colours and the stack size marked an assumption on no claim; `stack` out of the S2 list |
| F10 | fixed | §5 OQ5: no grouping above class is my choice, not S69's |
| F11 | fixed | §3: `edps` takes every range on the elemental line, the type tag ignored (S22) |
| F12 | fixed | §7 entry 3: without user names OQ5 types its classes out |
| F13 | fixed | §3, §4.6: `note` is the body's own field (S2); intent is `priced price` |
| F14 | fixed | §2, §4.5, §7 entry 7: the quantity is `holds(…)`; `count` names the view and the trade group only |
| F15 | fixed | §5 AQ2: OQ1's text alone |
| F16 | fixed | §5, §6: the sentences are the design's own |
| F17 | fixed | §3: the absence marked an inference from silence; S134 cited for S132 |

What the second audit taught me: my first repair patched sentences
without re-reading them against the rules beside them — two of these
slips (F3, F16) were mine from that pass.
