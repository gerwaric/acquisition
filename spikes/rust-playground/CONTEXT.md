...
This doc is working memory for coding agents and a place for developers to inspect and tweak things, not a spec or requirements document. It records what's settled, what's open, and what must never happen. Grows and changes structure as needed. Terse on purpose.
...
5. **Refresh tokens live in the OS keyring** (macOS Keychain / Windows Credential Manager / Secret Service). Never on disk in plaintext.

## Possible Decisions

The idea is that this section is append only, and each item is a short one-liner. As this grows we might need to pull it out of this file. The goal here is to optimize this stuff for agents. If there's another way of organizing this that will work better with agentic coding models, we should consider it.
...
- **Requests are jobs, not calls.** Rate-limit waits can reach 300s, so the core abstraction is a job with ID, state, and priority. Rationale: blocking calls can't represent a 5-minute wait sanely across three frontends.
...
- **Rate limiter: custom policy layer parsing GGG headers, with a simple enforcement mechanism underneath** (governor or semaphore + sleep). Rationale: GGG's header-driven limits are too specific for off-the-shelf policy.
- **Rate limiter spec will be expressed as test tables, not prose.** Existing heavyweight rate-limiter document is the input; "given these headers, wait N seconds" tests are the permanent, enforced spec.

## Possible Interfaces (boundaries are specified; internals are not)
...
ETA is computed from token-bucket state + queue depth ahead of the job — the daemon can predict, because it sees everything.
...
### acquisition-core public API

Not yet sketched. Deliberately open — expected to emerge from the first spike.
...
- Public vs confidential OAuth client?
...
- Design discussion precedes code on the `redesign` branch — but "design" means updating this doc, not writing a spec.
