...
- **Multi-account, remaining owner questions:** whether a session with no stored refresh token (a one-off login) should be listed as an account at all; whether a `refresh --all` may span accounts as one parent job or must be one job per account; and, for the GUI, whether the default account is per frontend or per daemon. Design is in "Multi-account design"; the Codex-side session that raised it is being consulted before build.
