...
- **Some item fields are volatile.** `veiledMods` placeholder ids change
  ...
  pulls an hour apart with nothing touched). The diff ignores them
...
- Caching API results in a local database that lives behind the daemon (persistence open topic, question 2).
- Job persistence across daemon restarts (SQLite-backed queue) (persistence open topic, question 1).
