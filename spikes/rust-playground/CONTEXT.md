...
One entry per decision: a stable id (`C<n>`, never reused; a superseded decision is rewritten in place under its id and dated, or deleted — git holds the history), the ruling in the owner's words, *Why:* in a sentence, and where the rest lives — *Details:* the module doc that carries the mechanism as recorded, *Pinned:* the tests, *Evidence:* the claim or ledger row. An entry is one bullet under `tools/docs-check.sh`'s length limit; a decision that needs more is a decision plus a mechanism, and the mechanism goes to the code. Two signals the limit gives (stage 5 of the search synthesis, 2026-09-17: eight of sixteen drafts over, one exactly on it): an entry that overflows is carrying mechanism, and an entry that lands on the limit is two decisions; an entry that enumerates — one clause per crate, per kind — grows per item, and the enumeration's home is the tool that enforces it (C1). Tests and docs cite decisions by id; the check refuses an id that does not exist and reports a decision nothing cites.
...
| `decisions/search.md` | Item search: the crate, the line, the query, the answer, the basis, the trade boundary | C89, C90, C91, C92, C93, C94, C95, C96, C97, C98, C99, C100, C101, C102, C103, C104 |
