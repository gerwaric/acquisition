...
#
# Run it when an edge is added to docs-check.sh (the split's step 3 adds
# the client and daemon crates), not in the gate: it runs the check
# once per case. Every case restores the files it touched from copies
...
files=(Cargo.toml Cargo.lock crates/acquisition-plan/Cargo.toml crates/acquisition-store/Cargo.toml crates/acquisition-cli/Cargo.toml tools/docs-check.sh)
...
crate bridge  external-bridge  0.1.0 "\n[dependencies]\nacquisition-core = { path = \"$root/crates/acquisition-core\" }\n"
...
plan_links_helper() { helper "$1"; adddep crates/acquisition-plan/Cargo.toml 'acquisition-helper = { path = "../acquisition-helper" }' "${2:-[dependencies]}"; }
...
none() { :; }
P='acquisition-plan → acquisition-helper → acquisition-core'
TR='links acquisition-core transitively'

echo '== the code: edges the planner must not have'
run_case 'clean tree'                                            pass   'ok      dependencies' none
run_case 'plan links core directly'                              refuse 'links acquisition-core —' adddep crates/acquisition-plan/Cargo.toml 'acquisition-core = { path = "../acquisition-core" }'
run_case 'plan → helper → core'                                  refuse "$P" plan_links_helper '[dependencies]\nacquisition-core = { path = "../acquisition-core" }\n'
run_case "helper → core under cfg(windows)"                      refuse "$P" plan_links_helper "[target.'cfg(windows)'.dependencies]\nacquisition-core = { path = \"../acquisition-core\" }\n"
run_case 'helper → core optional behind a feature'               refuse "$P" plan_links_helper '[dependencies]\nacquisition-core = { path = "../acquisition-core", optional = true }\n\n[features]\ndaemon = ["dep:acquisition-core"]\n'
run_case 'helper → external-bridge (optional, no member) → core' refuse 'acquisition-helper → external-bridge → acquisition-core' plan_links_helper "[dependencies]\nexternal-bridge = { path = \"$scratch/x/bridge\", optional = true }\n"
...
with_staged() { staged && "$@"; }
run_case 'the plan→core row replaced by a duplicate row'         refuse 'differs from the graph at row' with_staged shadow_table "perl -ne 'next if /^closure\\tacquisition-plan\\tacquisition-core\\t/; print; print if /^closure\\tacquisition-plan\\tacquisition-helper\\t/'"
run_case 'the row replaced by a fabricated row'                  refuse 'differs from the graph at row' with_staged shadow_table "perl -ne 'next if /^closure\\tacquisition-plan\\tacquisition-core\\t/; print; END { print \"closure\\tx\\ty\\tz\\tw\\tv\\n\" }'"
...
run_case 'the dependency name of the row renamed'                refuse 'differs from the graph at row' with_staged shadow_table "perl -pe 's/^closure\\tacquisition-plan\\tacquisition-core\\t/closure\\tacquisition-plan\\tmasked-core\\t/'"
run_case 'the member name of the row renamed'                    refuse 'differs from the graph at row' with_staged shadow_table "perl -pe 's/^closure\\tacquisition-plan\\tacquisition-core\\t/closure\\tmasked-plan\\tacquisition-core\\t/'"
...
run_case 'only the path column altered (diagnostic, unread)'     refuse "$TR" with_staged shadow_table "perl -pe 's/(^closure\\tacquisition-plan\\tacquisition-core\\t.*\\t).*$/\\1masked path/'"
