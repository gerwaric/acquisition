...
# header naming another acqd, another contract, a second header for one
# pid, and a provenance.json without the hashes. Each case runs the
...
expect "the second lifetime's header removed" fail "pid 2: POST oauth-token with no open header before it" "$work/header-removed.jsonl"
good | sed 's/{"pid":2,"method":"GET"/{"pid":3,"method":"GET"/' >"$work/unmatched.jsonl"
expect "a send whose pid has no header" fail "pid 3: GET stash@A#1 with no open header before it" "$work/unmatched.jsonl"
...
{ good; echo '{"event":"open","pid":2,"contract":"cccccccccccc","daemon":"dddd","clock":"system","ts":"t"}'; } >"$work/twice.jsonl"
expect "a second header for one pid" fail "pid 2: a second open header" "$work/twice.jsonl"
: >"$work/empty.jsonl"
