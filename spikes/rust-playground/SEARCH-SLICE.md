...
| 7 · computed values | `b5d62d92` | `totals.rs` and `reference/totals-v1.toml` in `acquisition-search`: the totals table as reference data (C94, C68) — 35 totals, 104 rows, the C++ app's pseudomod tables at `master@946a4f51` naming the trade site's pseudo stats, generated by `tools/totals-table.py`, compiled in, parsed once, checked by the loader; a row a line's group bound once (`group::row`), a total the evaluator's own sum weighted in halves and exact (`Exact::halved`), a ranged total built and pinned on a parsed table, none shipped; `pseudo.rs`: the namespace — a total's or a derived field's name, `pseudo.dps` and `pseudo.pdps` over the displayed properties (C101), the three statuses (complete, an incomplete subtotal, lacked; unavailable for the realm incomplete with the table's reason), comparing, sorting, summing and `undecided( … )` through the one scalar function, a row showing the value and the lines or properties counted; the basis gains `totals v1` (C98); `--describe` lists every computed value with its definition and the table's provenance. `pseudo.defence_pct` and a ranged total, `pseudo.<name>.<slot>`, refused by name, the entry naming no step. `tests/pseudo.rs`, `tests/answer.rs` (the worked example whole, every count as printed), `tests/acceptance.rs` (AQ2 as worded). M6 and M3 below; the holes at the plan's foot (T1–T4); the observations of step 7 below. |
...
The verdict on the recipe's rows: no plain resistance, attribute, speed
or gem-level line is uncounted, so the C++ app's rows stood; what the
trial found beyond them is the owner's (the plan's foot, T4). The site's
own answer on the ten fetched items of q5 (`search/trade-query/data/
fetch-census.json`) agrees with `total_fire_res` on every one, worked by
hand: 14 + 48 + 18 = 80, 18 + 48 + 15 = 81, 16 + 48 + 18 = 82, and the
seven with one line.
