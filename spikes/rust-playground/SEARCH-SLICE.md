...
- The copy's socket shapes: 9,687 `{group, attr, sColour}`, 47 poe2
  `{group, type}` (`gem` 46, `rune` 1), 4 `[]` (N44's hosts); `attr` and
  `sColour` agree on every socket; every group is one contiguous run, so
  the C++ run rule (S22) and GGG's number agree here; an abyssal socket
  (73 items) and a resonator's (7) is always a group of its own. Nothing
  unread: every unread shape is fixtures' and the generators'.
- A poe2 socket's `type` and a socketed gem's own `colour` and `socket`
  index are kept and asked by nothing; a term waits for a question.
