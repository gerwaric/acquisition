...
- `price_story.rs`'s `c35_two_blind_writers_on_one_row_never_clobber_and_the_receipts_chain`
  failed once under the parallel suite on 2026-09-11 ("round 1:
  annotation file … exists with content but no schema stamp; not
  opened") and passed six times alone and in every other full run that
  day (about a dozen). Two writers opening one annotations file at once
  is the test's subject, so the flake may be a first-open race in the
  annotations store worth a look by whoever next touches
  `annotations.rs` (C35); it is not the daemon split's. The round-24 fix
  commit landed on a chain that did not stop at that red exit — the
  gate-fix commit records it; the session-close skill's rule stands.
- A wedged pre-transition daemon whose backlog is saturated is read as
  ...
  step 6 that stopped accepting with a full backlog of 1024, on macOS.
