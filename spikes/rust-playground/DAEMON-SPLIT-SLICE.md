...
- C39 amended 2026-09-10 (the owner, verbatim: "change core + the
  ...
  bytes.
- `cargo build --workspace --all-targets` and `cargo build --workspace`
  write two different `target/debug/acqd` files and flip between them
  from cache in a tenth of a second (2026-09-11, verified: `a1217dc36951`
  against `20093483f1cd`): with every target, the daemon's dev-dependency
  on the store's `test-hooks` feature unifies into the daemon's own
  build, so the bin is another artifact. The gate's `cargo test
  --workspace --all-targets` does not touch the uplifted `acqd` (verified
  the same day), so the gate's order — build, then test — runs every
  daemon-owning test on the daemon the workspace build wrote. Two rules
  follow: build the daemon with the gate's `cargo build --workspace`, never
  the all-targets form, before anything that starts it; and never run
  the drivers while the gate runs — their preflight builds, and a build
  under a live daemon is an artifact mismatch (the round-16 gate saw
  exactly that). The `test-hooks` retirement the packet parked would
  remove the second artifact.
