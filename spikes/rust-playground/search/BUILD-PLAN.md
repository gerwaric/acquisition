...
| K1 | which colours a query names: the reference's four words; GGG's `A` (an abyssal socket, 93 on the copy) and `DV` (a resonator's, 16) are sockets of no word | every socket counts in `sockets` and is shown by `show`; `sockets.abyss` is an authoring error listing the four; an abyssal socket is asked for by the line every such item displays (`"Has # Abyssal Sockets"`), a resonator by its base | keep the four: the line and the base already select those items, and a fifth word is a value added to a closed list the day a question needs it |
