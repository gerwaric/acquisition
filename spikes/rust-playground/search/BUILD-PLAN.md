...
| T5 | the vocabulary whole is 954 ms at the audited build (M3 at `4237d2f3`): `line` alone lists every computed value on the second audit's advice, a reading of the reference's "whose name or definition matches" and no ruling, and the cost is that scan (630 ms by the table's rows), not the load. Does the persisted-projection park's trigger — "a CLI ask over 500 ms" — fire on a cost its candidates do not address, and does `line` alone list computed values? | the scan, bounded by the limit; every other release ask 275–360 | the trigger read by its purpose, the load: amend it to say so. `line` alone lists none, as the first audit round had it, and a narrowing lists those it matches (30–80 ms); the scan made cheap — the totals' rows indexed by the exact template each requires, the group still deciding — is a step 7b of 4b's shape, generators first, after the seat |
