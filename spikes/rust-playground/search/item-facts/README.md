...
Status: first pass complete — 2026-09-13. Census run over both stores; the map-identity and value-code questions are open.
...
- `baseType` joins the trade taxonomy for 78 % of items; every map fails (7,512: `baseType` is `Map (Tier N)`, the area lives only in the icon's art path) and so does every transfigured gem (272). PoE1 carries no class field; PoE2 carries one as property type 109 (F5).
...
- **Q1 — Map identity.** Is the area name anywhere but the icon's art path, and what do `mn`/`mg`/`mi` encode? Closes: one trade `fetch` of a map (its `name`/`typeLine`/`extended`), and the icon parameters of the 207 distinct map icons against the 282 trade map ids.
- **Q2 — The `values[][1]` code.** The reference says `uint`. Closes: the switch on it in the saved trade bundle's `PoE/Item/Popup` (colour classes), or the C++ `item.cpp` read.
- **Q3 — Transfigured gems and the taxonomy.** Which reference table maps a transfigured `baseType` to its gem? Closes: the trade `gem` group's `text`/`disc` entries against the 272 names.
...
Candidate claims for the ground truth (for the design session to route): the object line format and its dating; `influences` carrying four undocumented flags; maps carrying no area name; the `[Tag|Display]` leak; the veiled placeholder; ids surviving a league's end.

## Provenance

`MANIFEST.md` (every file, hash, source). Generated: `scripts/census.py` → `data/field-census.csv`, `data/mod-templates.csv`, `data/properties-census.csv`, `data/class-evidence.csv`, `data/numbers.md`; `scripts/drift.py` → F6 (printed); `scripts/line-format-dates.sh` → the F3 dating (printed; reads the backups in place). Hand-extracted: `data/api-reference-item-fields-2026-09-13.txt`. A fact not in a listed file is marked open.
