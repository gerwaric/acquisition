...
`MANIFEST.md`: the three clones at their pinned commits (every script refuses another), the files read in each, the sibling tracks' captures and stores, the owner's site check. Nothing here comes from memory of either project.
...
| `hash-recipe.md`, `hash-check.csv`, `stat-hashes.csv`, `twice-numbered.csv`, `twice-numbered-urls.txt` | `hash-check.py` | 6,090 / 1.5 MB; 6,958 / 0.5 MB; 260; 68 | the recipe and its verdict; every Path of Building number classed; stat id → `Stats.dat` hash; the twice-numbered stat ids with the hash's verdict, a search URL each (trade-query F9) and the owner's site verdict, read back from hand-kept `twice-numbered-verdicts.csv` |
...
Per category (`trade-stat-coverage.csv`, one row each): explicit 74.7 %, implicit 73.5 %, fractured 96.6 %, crafted 99.3 %, enchant 72.6 %, scourge 100 %; pseudo, imbued, mercenary, veiled, delve, ultimatum, sanctum and crucible 0–10 %, their ids being names, not stat hashes. The uncovered explicit ids are 1,685 `stat_` and 309 `indexable_`.
...
**F3 — Taxonomy** (`base-taxonomy.csv`, `class-to-trade-category.csv`). 103 classes in 66 categories; 82 trade ids = 14 union parents + 68 leaves: 58 are one class each, 7 need a name rule on the base (`Awakened `, `Cluster Jewel`, `Scarab`, `Invitation`, ` Fossil`, `Omen of `, `Tattoo of `), 3 (`monster.*beast`) are not bases. 31 classes have no trade id (quest, labyrinth, pantheon, sentinel, `DONOTUSE…`). Against `items.json`: 3,363 site base types agree with the predicted coarse category, 1 disagrees (a wombgift the export classes `RemovedItem`, as it does every graft), 8 unpredicted; the site's 586 unmatched types are beasts, numeric blighted-map ids and area-id charts. Corpus (PC, 35,651 items): 35,011 join by name (`Map (Tier N)` is an export base, so item-facts F5's maps join and only the area stays lost), 243 transfigured gems by stripping ` of …`, 397 do not (`Blighted Map (Tier N)`, beasts). The clipboard `Item Class:` equals the export's class name for all 46 listed base types.
...
**F5 — Many mods, one line** (the `mods.json` count). 4,369 prefix/suffix mods over 956 stat ids; 650 stat ids sit in two or more mods, 153 in ten or more (`base_maximum_life`: 55 mods in 9 groups). The game sums a stat id across mods and renders it once, so trade-query Q6's two-mod life line is summation; a private line's mods are recoverable only as the mod groups whose stat sets fit its value. Multi-id translations with trade ids are 237: 178 `minimum`/`maximum` pairs, 59 other.
