...
- **Q8 — Access method.** The C++ app fetches RePoE at runtime (cpp-search Q4); this track read a clone. What the design ships — a committed extract at a pinned commit, or nothing from RePoE — is the design session's ruling into `SURFACES.md`, together with how the search follows updates to the game, the API and the trade site (the owner, 2026-09-13: "this stays for the design session"). Folded in: whether to fork RePoE for the `Stats.dat` hash column, the fix that would make the trade-id mapping an algorithm and a rename-proof join — feasibility, cost and the recommendation to wait are `brainstorming-notes/20-repoe-hash-column-fork.md`; its trigger is the design choosing the stat id or the trade id as a line's identity and wanting rename-proof joins.
