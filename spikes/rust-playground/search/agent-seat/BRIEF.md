...
5. **Phase two — read-only SQL, after phase one is written down.** Only
   ...
   - engine-bench's projection at real scale,
     `<abs>/search/engine-bench/raw/bench-template-x1.db` (regenerated by
     its crate from the same two stores; read its `items` and `lines`
     DDL first — what it holds is what the bench needed, no more).
