...
- **Never open the store file with `sqlite3` or a script** (C48: raw
  SQL is not a surface). When the surfaces cannot answer and the file
  could, write down that you were tempted, and what you would have
  typed. That temptation is the finding.
...
5. **Close** as a full session does: the session-close skill, the index
...
for the CLI seat and the MCP seat where they differ.
