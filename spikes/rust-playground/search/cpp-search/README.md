...
Status: first pass complete — 2026-09-13. Every input read at `master@946a4f51`; the catalogues are under `data/`; the min/max accessors run over the 36,139-item corpus and the site's computed defences checked against the fetches (F2). The C++-side faults are master's, issue #229.

Headline:
- 38 filters in 9 groups over six payload kinds, ANDed in catalog order: 20 min/max, 11 flags, 2 text, 2 combos, 2 socket-colour, 1 mods. The min/max filters split on absence: eight property filters reject an item lacking the property, twelve read it as 0 (F1); three are dead against real payloads (F2).
...
| The item-facts stores and the trade-query fetches, for F2's accessor run | local, through their loaders | read |
...
**F2 — Derivations** (`data/derivations.md`, §2). Property strings are used as GGG sent them; only Quality (`+`/`%` stripped), Level (` (Max)` chopped) and Stack Size (`n/m` → n) are normalised. `properties` keeps `values[0][0]` only. Run over the corpus (`scripts/accessor-census.py` → `data/accessor-census.csv`, through item-facts' loaders): Crit. (3,269 items, every string `8.00%`-shaped) and Block (490, `23%`) parse as 0 on every item, so those **filters compare every item as 0** (a min above 0 rejects all, a bare max admits all; the columns sort because `Column::parts` tolerates the `%`), and **Map Tier reads a property no item carries** (0 of 7,273 maps; the tier is in the base type alone, item-facts F7), so an active Map Tier filter rejects every item. The other 17 parse on every item they are present on (links on 5,894 of 5,918 socketed items; the 24 are PoE2 sockets without `attr`). Against the site: `extended.ar/ev/es` on all 37 fetched defence values equal the property with the item's quality taken out and 20 % put in, within one; `_aug` is absent when the item already has 20 %.
...
| By-Item flat sort, real comparator (~530 ns a call, ⅔ of it two regexes) | 1,017 ms | 12,875 ms |
| Sort with materialized keys (27.5 MB / 266 MB of keys for one column) | 8.5 ms | 130 ms |
| Post-M3 unfiltered refilter, sort share 0; the bare `FilterItems` inside it | 23.6 / 20.2 ms | 282 / 329 ms |
| Delta into a visible By-Tab bucket; By-Item merge after the S5 fix | 0.87 / 3.9 ms | 0.86 / 32.7 ms |

Paid: dirty flags, the active-filter test per arrival, sorted merge into resident order, lazy rebuilds, budgeted key vectors, sort-on-expand, a visible-by-id index, one reconciliation per refresh. Accepted, not solved: the O(collection) filter pass and the O(n + d) flat merge. The report's §3 pins the implications: index the filter predicates (post-M3, ~93 % of a refilter is the unindexed scan); define ordering once and materialize keys, budgeted and scoped to what is visible; stable global item ids and container keys are load-bearing; one flat sorted result fights incremental update.
...
| Min/max filters dead against real payloads | 3 (Crit., Block, Map Tier) |

## Open questions

- **Q4 — RePoE as a surface.** The taxonomy and the dropdown depend on a third-party feed with no spike access method; repoe F3 generates the trade categories from the export's classes. Closes: repoe Q8, the design session's ruling into `SURFACES.md`.

## Provenance

Every file under `data/` names its inputs on its first line; all but one are extracts from `master@946a4f51` and regenerate by re-reading those files at that commit (`data/pseudomods.toml` by its script); `data/accessor-census.csv` is `scripts/accessor-census.py` over the item-facts stores and the trade-query fetches, local. Nothing here comes from memory of the app or the game; a rule not in a listed file is marked open. The pseudomod match read `../trade-query/data/stats-2026-09-12.json`.
