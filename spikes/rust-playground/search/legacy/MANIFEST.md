...
| --- | --- | --- |
| `PathOfBuilding/` | `16de4b82d57f1c0de6eb40f37143c32d4da36a02` | `src/Data/Uniques/*.lua` (22) and `src/Data/Uniques/Special/*.lua` (5) — the entries; `src/Data/Bases/*.lua` — the base names the parse checks a base line against; `src/Classes/Item.lua` — `ParseRaw`, for which lines its own parser reads as headers and for the catalyst rule (lines 15, 535-561, 660-860); `src/Data/EnchantmentHelmet.lua`, `src/Data/Crucible.lua` — for the sources read |
