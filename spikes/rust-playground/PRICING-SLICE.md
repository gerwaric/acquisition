...
- Opened at step 2 on 2026-09-03; re-scoped 2026-09-04. Annotations
  ...
  **6**; currency table **v1** (reviewed by the owner 2026-09-05);
  ...
  C68/MCP question, and the rigorous test pass, now **plan step 7**, open: item 1 (property
  tests on the three pure functions, proptest) landed 2026-09-07
  (`53421bd0`); item 2's writer landed the same day (`0f749018`) and
  waits on the owner's run — `cargo run -p acquisition-plan --example
  redact-pricing -- --league <name> --account <sel> >
  crates/acquisition-plan/reference/pricing-snapshot-<date>.json`,
  reviewed, committed; the suite's tests over it follow. Items 3 and 4
