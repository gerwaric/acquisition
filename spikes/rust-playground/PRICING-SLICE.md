...
3. The currency table's `status` is `"draft"` and its header says
   "Reviewed by: (nobody yet)": after you have read `currency-v1.toml`,
   set both by hand (or say so and an agent will). The loader gates
   nothing on it; the text surface prints it.
