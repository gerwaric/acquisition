...
`RUN-LEDGER.md`; facts about the trade site are the
...
| search step 9, the generators (2026-09-25) | `57c2f78b` | a body whose `note` is no string failed the pricing snapshot whole, and `acq price status` with it: unread at the note alone is the reading (the search plan's rule 8) | `ItemSnapshot::note_unread`, `GameSide::note_unread`; `the_pricing_snapshot_carries_notes_and_buyout_rows_verbatim_and_live_items_only` (`snapshot.rs`); `c81_a_note_that_cannot_be_read_leaves_the_price_unresolved` (`listing.rs`) |
| search step 9, the outside review (2026-09-25) | `1e0d85c6` | three: an `inventoryId` that is no string failed the snapshot the same way (1); an object note read as a string, SQLite's single-path extract flattening it to text (2); the unread-note override decided an item in a non-public tab, where C81 lets the game side say nothing (3) | (1), (2) `body_strings`, one two-path extract read by a flat scanner (its second look, `7ff3c947`: a decoder's depth limit had turned a deep field's sibling absent), `inventory_id_unread` (`snapshot.rs`), the snapshot test and the scanner's; (3) the override gated on `game.public`, the C81 test's second half |
