...
- C74's omission rule (an item the game's price already lists is
  ...
  (`Effective::side_word`: `game`, `manual`, `none`, `unresolved`), not
  `side` alone — `none` and `unresolved` both carry no side: a
  game-decided item is omitted and counted, a hand-decided one posted,
  one nothing applies to is not on the page, and an unresolved one (a
  row that cannot be read could decide) is blocked and counted, never
  treated as unpriced.
