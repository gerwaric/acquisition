...
| search step 9, the outside review (2026-09-25) | the review's commit | three: an `inventoryId` that is no string failed the snapshot the same way (1); an object note read as a string, SQLite's single-path extract flattening it to text (2); the unread-note override decided an item in a non-public tab, where C81 lets the game side say nothing (3) | (1), (2) `body_string`, one two-path extract, `inventory_id_unread` (`snapshot.rs`), the snapshot test; (3) the override gated on `game.public`, the C81 test's second half |
